...
and `WalkForward` cross-validates. Names label the leaderboard, never the
wiring.
...
the shape of the signal. Inside a family the members trade off linearity,
...
|---|---|---|
| **Point** | `SklearnPoint`, `OnlineAggregator`, `RNNHourly` | a single μ̂ per row, lifted to a distribution by a [Lifter](#lifters) |
...
| **Baselines** | `Persistence`, `PersistenceDist`, `EmpiricalDistribution` | reference forecasts to beat, plus the convenience factories `ridge` and `emos_calibrated` |
...
- **`HeteroscedasticNormal`** generalises it to the features, with
  `μ = Xμ·βμ` and `log σ = Xσ·βσ`. Any columns (cloud, wind, dewpoint, spread,
  …) drive **both** the location and the width, with readable linear
  coefficients. `EMOS` is the special case `Xμ=[ens_mean]`, `Xσ=[ens_std]`.
- **`NGBoostNormal`** targets the same `(μ̂, σ̂)`-from-features but
  gradient-boosts it, so it is non-linear, noisier at low N, and not
  interpretable. Reach for it when the relationship bends and the rows are
  there.
...
distribution skews or otherwise departs from Gaussian and committing to a
parametric family is undesirable. The [tail policy](tail_policies.md) governs
behaviour past the outermost quantile.

### Distribution-first vs bracket-aware (fit interface)

A second axis cuts across the families, namely what a trainer sees at fit
time.
...
- **Bracket-aware** (`CumulativeBinary`, `BlendedBracketGBM`/`BlendedBracketNet`,
  ...
  take their grids (`cutpoints_by_id`/`outer_edges_by_id`, respectively
  ...
  and a `Pipeline`/`WalkForward` forwards `ids` and the call-time grids.
...
- **`StackedParametric`** is a parametric meta-learner over upstream means and
  sigmas.
- **`BMAStacking`** is Bayesian model averaging, a likelihood-weighted mixture.
- **`LinearPoolDist`** is a linear opinion pool, the weighted average of CDFs.
- **`BracketStacking`** stacks in bracket-probability space.
- **`CDFBoostBracket`** boosts a base CDF on bracket residuals.
- **`TailSpecialist`** is a second model dedicated to the outer brackets,
...
|---|---|---|
| `GroupByZScore` | per-group standardized anomaly, learning scale = `std(y − center)` per group (e.g. per station), mapping `(v − center)/scale`, and inverse-mapping the predicted dist back to real units | the proven weather win, which strips a per-group location/scale confound. Use `level_cols=()` for **target-only** z-scoring, the right mode when X holds mixed columns and the model is scale-invariant (trees) |
| `IdentityTransformer` | no-op pass-through | the shim shape for plugging a plain sklearn X-only transformer into a pipeline (override `transform`, leaving target and inverse as identity), or an explicit "normalise nothing" marker |
...
`GroupByZScore` also takes `spread_cols`, which are divide-only and suit an
ensemble std, and `passthrough_cols`, which are left untouched and suit binary
missing-flags.
...
| `GARCHResidual` | GARCH(1,1) volatility recursion → per-row σ (one-step), with optional Student-t innovations | residual **volatility clusters** over time (returns, vol regimes). Each row gets its own σ from the fitted history |
...
set. They differ by which representation they correct, so pick the one matching
the forecaster's backing.

| Calibrator | Operates on | What it fixes | When to use |
|---|---|---|---|
| `Isotonic` | bracket probabilities | a single monotone curve on (predicted prob → realized hit), per cell, then row-renormalised | systematic over/under-statement of bracket probabilities. Grid-agnostic, since `pre_integrate_edges=` auto-integrates a non-bracket dist first |
| `PITCalibrate` | the predictive CDF (any backing) | isotonic recalibration of the PIT `u = F̂(y)` toward Uniform(0,1) | whole-distribution miscalibration, whether a U-shaped PIT (over-confident) or a humped one (under-confident) |
| `ConformalCalibrate` | quantile backings only | per-τ offsets giving finite-sample `(1−τ)` coverage under exchangeability (CQR, Romano 2019) | quantile forecasters needing a coverage *guarantee*. It rejects non-quantile dists loudly |
...
| `MultiOutput` | `bracketlearn.multitarget` | wraps a single-target model for `(N, M)` targets. See [multitarget](multitarget.md) |
