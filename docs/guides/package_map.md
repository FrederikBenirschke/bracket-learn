...
splits into four layers: **compose** (building and cross-validating models),
**data types** (what a forecast *is*), **trainers** (what produces a forecast),
and **pricing / scoring** (what you do with one).
...
| Module | Owns |
|---|---|
| `bracketlearn.forecast` | `DistributionForecast` (ABC) plus the five backings (`NormalForecast`, `StudentTForecast`, `MixtureNormalForecast`, `QuantileForecast`, `BracketForecast`), plus `PointForecast`, `ContractForecast`, `ProvenanceMeta`, `TailPolicy`. Each backing owns its own CRPS / CDF / `integrate` math. |
